...
def test_doc_output(
    ...
    logging.info("Python CosmosDB output binding function processed a request.")
...
def create_user(
    req: func.HttpRequest, outputDocument: func.Out[func.Document]
) -> func.HttpResponse:
...
@user_bp.route(route="get_user", methods=["POST"])
def get_user(req: func.HttpRequest) -> func.HttpResponse:
    """This function takes a username from the request and returns the user from the CosmosDB container.
    ...
    Returns:
        func.HttpResponse: The response to the request.
    """
    logging.info("Python HTTP get_user function processed a request.")
    user_info = req.get_json()
    if (
        not "username" in user_info
        and not "password" in user_info
        and not "salt" in user_info
    ):
        try:
            req_body = req.get_json()
        except ValueError:
    ...
    if username and password and salt:
        ...
        potential_pass = hash(password, salt)
        params = [
        ...
        return func.HttpResponse(
            "User retrieval failed. Please provide a valid username and password.",
            status_code=HTTPStatus.NOT_FOUND,
        )
    else:
        return func.HttpResponse(
            "User retrieval failed. Please provide a username, password, and salt in the request body.",
            status_code=HTTPStatus.BAD_REQUEST,
        )
...
def check_user_exists(req: func.HttpRequest) -> func.HttpResponse:
    ...
    if user_info and "username" in user_info and "email" in user_info:
        ...
        if list(items) != []:
            return func.HttpResponse(
                json.dumps({"userExists": True}),
                status_code=HTTPStatus.FORBIDDEN,
                mimetype="application/json",
            )
        else:
            return func.HttpResponse(
                json.dumps({"userExists": False}),
                status_code=HTTPStatus.OK,
                mimetype="application/json",
            )
    else:
...
def query_cosmos_db(
    query: str, params: list, container: ContainerProxy, cross_part=False
) -> ItemPaged:
